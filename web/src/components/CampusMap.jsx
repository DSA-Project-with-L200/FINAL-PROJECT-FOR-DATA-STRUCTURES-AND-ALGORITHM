...
function computeDijkstraPath(sourceId, destId) {
  if (sourceId === destId) {
    return {
      distance: 0,
      pathNodeCount: 1,
      pathNames: [CAMPUS_LOCATIONS.find(l => l.id === sourceId)?.name || 'Same'],
...
// Modal Component for Realtime Ride Mapping & Turn-by-Turn Navigation
function RealtimeRideModal({ dispatch, onClose, onLog }) {
  ...
  const pickupLoc = CAMPUS_LOCATIONS.find(l => l.id === dispatch.request.pickupLocation) || CAMPUS_LOCATIONS[0];
  const destLoc = CAMPUS_LOCATIONS.find(l => l.id === dispatch.request.destLocation) || CAMPUS_LOCATIONS[1];
  const driverLoc = CAMPUS_LOCATIONS.find(l => l.id === dispatch.driver.locationId) || pickupLoc;

  // Build full coordinate points sequence for map rendering
  const routeNodeIds = dispatch.route.pathNodeIds && dispatch.route.pathNodeIds.length > 0
    ? dispatch.route.pathNodeIds
    : [dispatch.request.pickupLocation, dispatch.request.destLocation];

  const routeLocations = routeNodeIds.map(id => CAMPUS_LOCATIONS.find(l => l.id === id)).filter(Boolean);
  const routeLatLngs = routeLocations.map(l => [l.lat, l.lng]);
  ...
  useEffect(() => {
    ...
    // Draw Dijkstra Shortest Path Polyline
    ...
    const driverIcon = L.divIcon({
      ...
      html: `<div style="background:#0284c7; color:#fff; width:28px; height:28px; border-radius:50%; border:3px solid #fff; box-shadow:0 0 15px #0284c7; display:flex; align-items:center; justify-content:center; font-weight:bold; font-size:14px;">🚕</div>`,
      iconSize: [28, 28],
      iconAnchor: [14, 14],
    });

    driverMarkerRef.current = L.marker([driverLoc.lat, driverLoc.lng], { icon: driverIcon }).addTo(map);
    driverMarkerRef.current.bindTooltip(`<b>🚕 ${dispatch.driver.name}</b><br>${dispatch.driver.vehicle}`, { permanent: true, direction: 'top' });

    // Pickup Location Pin
    const pickupIcon = L.divIcon({
      className: 'custom-pickup-marker',
      html: `<div style="background:#f59e0b; color:#fff; width:26px; height:26px; border-radius:50%; border:2px solid #fff; box-shadow:0 0 12px #f59e0b; display:flex; align-items:center; justify-content:center; font-size:12px;">📍</div>`,
      iconSize: [26, 26],
      iconAnchor: [13, 13],
    });
    L.marker([pickupLoc.lat, pickupLoc.lng], { icon: pickupIcon }).addTo(map)
      .bindTooltip(`<b>📍 Pickup: ${dispatch.request.requesterName}</b><br>${dispatch.pickupName}`, { permanent: false });

    // Destination Location Pin
    const destIcon = L.divIcon({
      className: 'custom-dest-marker',
      html: `<div style="background:#f43f5e; color:#fff; width:26px; height:26px; border-radius:50%; border:2px solid #fff; box-shadow:0 0 12px #f43f5e; display:flex; align-items:center; justify-content:center; font-size:12px;">🏥</div>`,
      iconSize: [26, 26],
      iconAnchor: [13, 13],
    });
    L.marker([destLoc.lat, destLoc.lng], { icon: destIcon }).addTo(map)
      .bindTooltip(`<b>🏥 Destination: ${dispatch.destName}</b>`, { permanent: false });
  ...
  // Handle Driver Vehicle Live Progress Animation
  useEffect(() => {
    let interval = null;
    if (isPlaying) {
      interval = setInterval(() => {
        setAnimProgress(prev => {
          if (prev >= 100) {
            setIsPlaying(false);
            if (onLog) onLog('RIDE_TRAVERSAL_COMPLETE', `🏁 Ride ${dispatch.id} arrived at destination: ${dispatch.destName}`);
            return 100;
          }
          return prev + 2.5;
        });
  ...
  // Update vehicle position on map along interpolation path
  useEffect(() => {
  ...
  const etaMinutes = Math.ceil(currentDistRemaining / 150);
    ...
    >
      ...
      >
        ...
        >
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <span style={{ background: '#10b981', color: '#fff', fontSize: '0.7rem', fontWeight: 800, padding: '2px 8px', borderRadius: '4px' }}>
                REALTIME DIJKSTRA NAVIGATION
              </span>
              <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 800 }}>
                Ride Mapping & Turn-by-Turn Directions: {dispatch.id}
              </h3>
            </div>
            <p style={{ fontSize: '0.775rem', color: '#94a3b8', marginTop: '2px' }}>
              Linking <b>{dispatch.driver.name}</b> ({dispatch.driver.vehicle}) ➔ <b>{dispatch.request.requesterName}</b> ({dispatch.request.role})
            </p>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', flex: 1, overflowY: 'auto' }}>
          {/* Map Viewport Side */}
          <div style={{ position: 'relative', height: '420px', minHeight: '350px' }}>
            <div ref={mapContainerRef} style={{ width: '100%', height: '100%' }} />

            {/* Floating Ride Overlay Banner */}
            <div
              style={{
                position: 'absolute',
                bottom: '12px',
            ...
            >
              <div>
                <div style={{ fontSize: '0.65rem', color: '#94a3b8', textTransform: 'uppercase', fontWeight: 700 }}>LIVE GPS RIDE STATUS</div>
                <div style={{ fontSize: '0.9rem', fontWeight: 800, color: '#38bdf8' }}>
                  {animProgress === 100 ? '✅ Arrived at Destination' : `In Transit: ${currentDistRemaining}m remaining (~${etaMinutes} mins ETA)`}
                </div>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                >
                  ...
                  {isPlaying ? 'Pause Simulation' : '▶ Simulate Ride'}
                </button>
              </div>
            </div>
          </div>

          {/* Turn-by-Turn Navigation & Ride Info Side */}
          <div style={{ padding: '1.25rem', background: '#f8fafc', overflowY: 'auto' }}>
            {/* Requester & Driver Info Summary Card */}
            <div style={{ background: '#ffffff', padding: '12px', borderRadius: '10px', border: '1px solid #e2e8f0', marginBottom: '1rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                <span style={{ fontSize: '0.7rem', fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase' }}>
                  VEHICLE & PASSENGER TELEMETRY
                </span>
                <span style={{ fontSize: '0.7rem', background: '#dcfce7', color: '#15803d', padding: '2px 8px', borderRadius: '12px', fontWeight: 'bold' }}>
                  Speed Limit: 30 km/h
                </span>
              </div>

              <div style={{ fontSize: '0.85rem', color: 'var(--text-main)', lineHeight: '1.5' }}>
                <div>🚕 <b>Vehicle:</b> {dispatch.driver.vehicle} ({dispatch.driver.name})</div>
                <div>📞 <b>Contact:</b> {dispatch.driver.phone || '+233 24 123 4567'}</div>
                <div>👤 <b>Passenger:</b> {dispatch.request.requesterName} ({dispatch.request.role})</div>
                <div>📍 <b>Pickup:</b> {dispatch.pickupName} ➔ 🏥 <b>Dest:</b> {dispatch.destName}</div>
              </div>
            </div>

            {/* Dijkstra Turn-by-Turn Navigation Traversal Steps */}
            <div style={{ background: '#ffffff', padding: '12px', borderRadius: '10px', border: '1px solid #e2e8f0' }}>
              <div style={{ fontSize: '0.75rem', fontWeight: 800, color: 'var(--accent-emerald)', textTransform: 'uppercase', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                <Compass size={16} /> Dijkstra Shortest Path Directions ({dispatch.route.pathNodeCount} Nodes)
              </div>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {dispatch.route.pathNames.map((nodeName, idx) => (
                  <div
                    key={idx}
                    style={{
                      display: 'flex',
                      ...
                      fontSize: '0.775rem',
                      padding: '6px 8px',
                      background: idx === 0 ? '#f0f9ff' : idx === dispatch.route.pathNames.length - 1 ? '#fff1f2' : '#f8fafc',
                      borderRadius: '6px',
                      borderLeft: idx === 0 ? '3px solid #0284c7' : idx === dispatch.route.pathNames.length - 1 ? '3px solid #f43f5e' : '3px solid #10b981'
                    }}
                  >
                    <span style={{ fontWeight: 800, color: 'var(--text-muted)' }}>{idx + 1}.</span>
                    <div>
                      <div style={{ fontWeight: 700, color: 'var(--text-main)' }}>
                        {idx === 0 ? `Start: ${nodeName}` : idx === dispatch.route.pathNames.length - 1 ? `Arrive Destination: ${nodeName}` : `Proceed via ${nodeName}`}
                      </div>
                      <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)' }}>
                        {idx === 0 ? `Driver departing station #${dispatch.driver.locationId}` : `Optimal road segment (${Math.round(dispatch.route.distance / dispatch.route.pathNodeCount)}m)`}
                      </div>
                    </div>
                  </div>
                ))}
...
export default function DispatchEngine({ onLog }) {
  ...
  // Selected request for formula inspection & selected ride for live mapping modal
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                {activeDispatches.map(dsp => (
                  <div
                    ...
                    onClick={() => setSelectedRideModal(dsp)}
                  >
                    ...
                    <div style={{ marginTop: '0.75rem', display: 'flex', gap: '0.5rem', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' }}>
                      <button
                        className="btn btn-primary"
                        style={{ width: 'auto', padding: '0.4rem 0.85rem', fontSize: '0.75rem' }}
                        onClick={(e) => {
                          e.stopPropagation();
                          setSelectedRideModal(dsp);
                        }}
                      >
                        <Compass size={14} /> 🗺️ View Realtime Navigation & Directions
                      </button>
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          completeDispatch(dsp.id);
                        }}
                      >
                        <CheckCircle2 size={14} style={{ color: 'var(--accent-emerald)' }} /> Complete Ride
                      </button>
                    </div>
                  </div>
                ))}
