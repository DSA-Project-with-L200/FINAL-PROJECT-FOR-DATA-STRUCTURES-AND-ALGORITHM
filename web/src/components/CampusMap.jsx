...
import { ZoomIn, ZoomOut, RotateCcw, Navigation, Route, Layers } from 'lucide-react';
import L from 'leaflet';
import 'leaflet/dist/leaflet.css';

export default function CampusMap({ onLog, activeSimulationRequest }) {
  ...
  // Handle active simulation request from Dispatch Engine
  useEffect(() => {
    if (activeSimulationRequest) {
      if (activeSimulationRequest.pickupLocation) {
        setSourceNode(Number(activeSimulationRequest.pickupLocation));
      }
      if (activeSimulationRequest.destLocation) {
        setTargetNode(Number(activeSimulationRequest.destLocation));
      }
    }
  ...
  const runDijkstra = (src, tgt) => {
    ...
    const haversineDist = srcLoc && tgtLoc ? calculateHaversineDistance(srcLoc.lat, srcLoc.lng, tgtLoc.lat, tgtLoc.lng) : 0;

    setRouteInfo({
      roadDistance: dist[tgt],
      haversineDistance: haversineDist,
      pathNames: path.map((id) => CAMPUS_LOCATIONS.find((l) => l.id === id)?.name || `#${id}`),
      srcCoords: srcLoc ? `${srcLoc.lat}, ${srcLoc.lng}` : '',
      tgtCoords: tgtLoc ? `${tgtLoc.lat}, ${tgtLoc.lng}` : '',
    });

    if (onLog && srcLoc && tgtLoc) {
      ...
      {routeInfo && (
        <div className="card" style={{ marginTop: '1rem', borderColor: 'var(--accent-emerald)' }}>
          <div className="card-title" style={{ color: 'var(--accent-emerald)' }}>
            <span><Route size={18} style={{ marginRight: '6px', verticalAlign: 'middle' }} /> Google Maps Dijkstra Route Results</span>
